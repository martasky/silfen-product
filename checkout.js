const CART = {
  ...
  updateDOM() {
    const checkoutcontentEl = document.querySelector(
      ".purchase-summary-item-container"
    );
    ...
    if (CART.contents.length === 0) {
      ...
      CART.contents.forEach((element) => {
        ...
        const tempItem = document.querySelector("#checkout-template").content;
        ...
        itemcopy.querySelector(".item img").src = element.images[1];
        itemcopy.querySelector(".color").textContent = edit_color_name(
          element.colors[0].name
        );
        itemcopy.querySelector("h4").textContent = element.name;
        ...
        checkoutcontentEl.appendChild(itemcopy);
      });
